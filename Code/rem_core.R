...
#         lambda      =       final lambda value - will decrease from infinity to lambda
#                             lambda will automatically equal Inf, if not specified
...
#         inits       =       Initial condition for EM. It contains cluster average, 
#                             covariance matrices, and mixing proportions for clusters. 
#
EM_robust = function(sampleMat, c, lambda = Inf, d, sigma_str, inits) {
    ...
    for (l in ll) {
        ...
        for (it in 1:max_it) {
            ...
            for (j in 1:c) {
                ...
                if (sigma_str == "unstr") {
                  ...
                  for (m in 1:length(indices)) {
                    tempSig = as.numeric(T_mat[j, indices[m]] * t(matrix(x_prime[m,])) %*% matrix(x_prime[m,])) * diag(d)
                    sigma[[j]] = sigma[[j]] + tempSig
                  }
                  sigma[[j]] =  matrix(sigma[[j]]/denom, d, d)
                }
                # Diagonal structure
                else {
                    x_prime = x[indices,]-matrix(mu[j,], ncol=d, nrow=length(indices), byrow=T)
                    sigma[[j]] = 0
                    for (m in 1:length(indices)) {
                      tempSig = as.numeric(T_mat[j, indices[m]] * x_prime[m,]^2) * diag(d)
                      sigma[[j]] = sigma[[j]] + tempSig
                    }
                    denom = sum(T_mat[j,indices])
                    sigma[[j]] =  matrix(sigma[[j]]/denom, d, d)
            ...
            ## take care of inf values (constant/0.00... cases)
    ...
    for (j in 1:c) {
        ...
        MHdist = matrix(rowSums(x1_sigma_inv * x1), n, 1)
        for (i in 1:n) {
            ## This is a improvable place. The cutoff value can be taken from chisq distribution. 
            if (MHdist[i,] < lam^2) {
